...
/* ---------------------------------------------------------------------------------- */
void thermal_analysis_MM(design)
struct Design *design;
{
  double *flux, **R;
  void set_up_tfield(), set_up_flux(), set_up_coef_matrix();
  void gauss_seidel(), LU_Decomp(), find_comp_temp(), find_contained_nodes();
  ...
  else LU_Decomp(design, R, flux, tot_nodes, hbw);
...
int node_dim[];
{
  ...
  printf("%d %d %d  ", node_dim[0], node_dim[1], node_dim[2]);
