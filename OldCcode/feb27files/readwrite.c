...
struct Design *design;
{
  ...
  fptr = fopen("output/design.data","r");
  i = 0;
  comp = design->first_comp;
  while (++i <= COMP_NUM)
    {
      ...
      fscanf(fptr, "%lf %lf", &(comp->half_area), &(comp->mass));
      if (i < COMP_NUM)
	comp = comp->next_comp;
    }
...
struct Design *design;
{
  ...
  for (k = 0; k < NODE_NUM; ++k)
    fprintf(fptr, "%lf %lf %lf  %lf\n", design->tfield[k].coord[0], design->tfield[k].coord[1], 
	    design->tfield[k].coord[3], design->tfield[k].temp);
  fclose(fptr);
}

/* ---------------------------------------------------------------------------------- */
/* This function reads the final temperature field from a file.                       */
/* ---------------------------------------------------------------------------------- */
...
struct Design *design;
{
  ...
  printf("Restoring saved tfield\n");
  fptr = fopen("output/tfield.data","r");

  for (k = 0; k < NODE_NUM; ++k)
    fscanf(fptr, "%lf %lf %lf", &(design->tfield[k].coord[0]), &(design->tfield[k].coord[1]), &(design->tfield[k].temp));
  fclose(fptr);
