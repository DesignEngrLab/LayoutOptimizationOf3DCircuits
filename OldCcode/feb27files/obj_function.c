...
/* ---------------------------------------------------------------------------------- */
double evaluate(design)
struct Design *design;
{
  void eval_part_1(), eval_part_2(), eval_part_3(), eval_part_5(), update_coef();
  int i, k;
  double eval;
  char wait;
  FILE *fptr;
...
/* Add up the individual evaluations. */
  ...
  for (i = 0; i < OBJ_NUM; ++i) {
    if (design->new_obj_values[i] != 0.0)
      eval += design->weight[i]*design->new_obj_values[i];/*design->coef[i];*/
  }
  printf("%f %f %f   %f\n", design->new_obj_values[0], design->new_obj_values[3],
	    design->new_obj_values[4],  eval);
