...
struct Design *design;
{
  ...
  double calc_temp_penalty(), calc_cool_obj(), calc_thermal_matrix();
  double tempave;
...
/*  printf("%.2f\n", tempave);*/
  ...
  for (i = 1; i <= COMP_NUM; ++i) {
    design->new_obj_values[3] = calc_temp_penalty(tempave, comp->tempcrit);
    design->new_obj_values[4] = calc_cool_obj(tempave, comp->tempcrit);
    if (i < COMP_NUM)
      comp = comp->next_comp;
  }
}
...
struct Design *design;
{
  ...
  Tave = (design->tamb) + (Qtot*Rtot)/design->hcf;
...
{
  struct Component *comp;
  double *flux, **R, calc_temp_penalty();
