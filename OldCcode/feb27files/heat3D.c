...
struct Design *design;
{
  ...
  void calc_thermal_matrix_gauss(), calc_thermal_matrix_LU();
...
struct Design *design;
{
  ...
  gauss_seidel(design, R, flux, tot_nodes, hbw, node_dim[2], design->tolerance);
...
int k;
{
  ...
  comp = first_comp;
  while (comp != NULL) {
    if ((comp->coord[0] == tfield[k].coord[0]) && 
	(comp->coord[1] == tfield[k].coord[1]) &&
        (comp->coord[2] == tfield[k].coord[2])) {
	    comp->node_center = k;
	    return;
	}
    comp = comp->next_comp;
...
/* ---------------------------------------------------------------------------------- */
void gauss_seidel(design, R, flux, tot_nodes, hbw, znodes, tolerance)
struct Design *design;
double **R, *flux, tolerance;
int tot_nodes, hbw;
{
    ...
    } while ((tol > tolerance) && (iter < 1000));
    printf("Iterations = %d: Component #1 temperature = %.2f  %f\n", iter, 
	design->tfield[0].temp, tol);
}

/* ---------------------------------------------------------------------------------- */
/* This is function establishes the intial guesses used by Gauss-Seidel.              */
/* ---------------------------------------------------------------------------------- */
...
struct Design *design;
{
  double *flux, **R, calc_thermal_penalty();
  void set_up_tfield(), set_up_flux(), set_up_coef_matrix(), back_up_tfield();
  void matrix_solver(), find_comp_temp(), find_contained_nodes();
  int i, j, node_dim[DIMENSION], tot_nodes, hbw, width;

  printf("Calculating Thermal Matrix using LU-Decomposition.\n");
  set_up_tfield(design, node_dim);
  hbw = node_dim[1]*node_dim[2];
  width = 2*hbw + 1;
  tot_nodes = node_dim[0]*hbw;
