...
struct Design *design;
{
  ...
  void update_heat_param(), back_up_tfield(), revert_tfield();
...
/* Take a step and evaluate it.  Update state by accepting or rejecting step. */
	  ...
	  step_eval = evaluate(design, 
	    ((steps_at_t - 1)%((gen_limit/design->hcf_per_temp) + 1)));
  ...
  while(++i < MOVE_NUM)
      ...
      if (prob < 0.)
	{
	  ...
	  if (i < TRANS_NUM) {
	    ...
	    if (hustin->move_size[i] < design->maxmove) 
		design->small_move = 1;
	  }
