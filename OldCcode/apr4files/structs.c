...
/* hcf is the heatapp correction factor.                                              */
/* hcf_per_temp is the number of times heatapp gets corrected per temperature.        */

  struct Temperature_field tfield[NODE_NUM*NODE_NUM*NODE_NUM];
  double kb, h[3], tamb, tolerance, min_node_space, hcf; 
  int hcf_per_temp, small_move, maxmove, gauss, max_iter;
};
