...
/*  matrix_solver3(design, R, flux, tot_nodes, hbw, node_dim[2]);*/
...
/* This is the matrix solver for the vector of nodes in tfield.  It uses LU decomp.   */
/* ---------------------------------------------------------------------------------- */
void matrix_solver2(design, R, flux, n, hbw, znodes)
struct Design *design;
double **R, *flux;
int n, hbw, znodes;
{
...
/*   L-U Decomposition of Banded Matrix; returning values to the R banded matrix  */
  ...
  for (k = 1; k < (n-1); ++k) {
    for (i = 0; i <= 3; ++i) {
      if ((k+pos[i]) < n) {
	sum = R[k+pos[i]][c-pos[i]];
	for (j = 1; j <= (3-i); ++j)
	  if (pos[j] <= k) sum -= R[k-pos[j]][c+pos[j]]*R[k+pos[i]][c-pos[i]-pos[j]];
	R[k+pos[i]][c-pos[i]] = sum;
      }
    }
    for (i = 1; i <= 3; ++i) {
      sum = R[k][c+pos[i]];
      for (j = 1; j <= (3-i); ++j)
        if (pos[j] <= k) sum -= R[k-pos[j]][c+pos[j]+pos[i]]*R[k][c-pos[j]];	   
      R[k][c + pos[i]] = sum/R[k][c];
    }
  }

  sum = R[n-1][c];
  for (i = 1; i <= 3; ++i)
    sum -= R[n-1][c-pos[i]]*R[n-1-pos[i]][c+pos[i]];
  R[n-1][c] = sum;
...
/*   L-U Back-Substition to get temperatures.   */
  for (k = 0; k < n; ++k) {
    sum = flux[k];
    for (i = 1; i <= 3; ++i)
      if (pos[i] <= k) sum -= R[k][c-pos[i]]*flux[k-pos[i]];
    flux[k] = sum/R[k][c];
  }
  for (k = n-1; k >= 0; --k) {
    sum = flux[k];
    for (i = 1; i <= 3; ++i)
      if ((k+pos[i]) < n) sum -= R[k][c+pos[i]]*(design->tfield[k+pos[i]].temp);
    design->tfield[k].temp = sum;
  }
}
...
/* This is the matrix solver for the vector of nodes in tfield.  It uses LU decomp.   */
/* ---------------------------------------------------------------------------------- */
void matrix_solver3(design, R, flux, n, hbw, znodes)
struct Design *design;
double **R, *flux;
int n, hbw;
{
...
/*   L-U Decomposition of Banded Matrix; returning values to the R banded matrix  */
  for (k = 1; k < (n-1); ++k) {
    for (i = 0; i <= 6; ++i) {
      if ((k+pos[i]) < n) {
	sum = R[k+pos[i]][c-pos[i]];
	for (j = 1; j <= (hbw-pos[i]); ++j)
	  if (j <= k) sum -= R[k-j][c+j]*R[k+pos[i]][c-pos[i]-j];
	R[k+pos[i]][c-pos[i]] = sum;
      }
    }
    for (i = 1; i <= 6; ++i) {
      sum = R[k][c+pos[i]];
      for (j = 1; j <= (hbw-pos[i]); ++j)
        if (j <= k) sum -= R[k-j][c+j+pos[i]]*R[k][c-j];	   
      R[k][c + pos[i]] = sum/R[k][c];
    }
  }
