...
struct Design *design;
{
  ...
  for (k = 0; k < NODE_NUM; ++k)
    fprintf(fptr, "%lf %lf %lf\n", design->tfield[k].x_coord, design->tfield[k].y_coord, design->tfield[k].temp);
  fclose(fptr);
}
...
struct Design *design;
{
  ...
  for (k = 0; k < NODE_NUM; ++k)
    fscanf(fptr, "%lf %lf %lf", &(design->tfield[k].x_coord), &(design->tfield[k].y_coord), &(design->tfield[k].temp));
  fclose(fptr);
