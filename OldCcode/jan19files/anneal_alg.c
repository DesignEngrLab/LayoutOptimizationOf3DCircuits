...
struct Design *design;
{
  ...
  void init_anneal(), init_schedule(), take_step(), update_temp(), fprint_data();
  ...
  void update_tolerance(), back_up_tfield(), revert_tfield();
...
/* Take a step and evaluate it.  Update state by accepting or rejecting step. */
	  ...
	  step_eval = evaluate(design, iteration);
...
/*	      if (current_eval > step_eval)
		...
		hustin->delta_c[hustin->which_move] += (step_eval - current_eval)/5.0;*/
	      ...
	      update_coef(design, &column, &cost_update);
...
/*	  update_tolerance(design, iteration);     IN HEAT.C */
	  update_weights(design, t);          /* IN OBJ_BALANCE.C */
...
/* Print out evaluation information about the last design. */
  ...
  printf("The final eval was %lf (%lf percent density)\n",step_eval,(100./step_eval));

  if (design->new_obj_values[3]) printf("****       Still a thermal violation!       ***** %.1f\n", design->new_obj_values[3]);
